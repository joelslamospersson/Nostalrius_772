...
Game::Game()
{
	...
	offlineTrainingWindow.choices.push_back(std::make_pair("Sword Fighting and Shielding", SKILL_SWORD));
	offlineTrainingWindow.choices.push_back(std::make_pair("Axe Fighting and Shielding", SKILL_AXE));
	offlineTrainingWindow.choices.push_back(std::make_pair("Club Fighting and Shielding", SKILL_CLUB));
	offlineTrainingWindow.choices.push_back(std::make_pair("Distance Fighting and Shielding", SKILL_DISTANCE));
	offlineTrainingWindow.choices.push_back(std::make_pair("Magic Level and Shielding", SKILL_MAGLEVEL));
	offlineTrainingWindow.buttons.push_back(std::make_pair("Okay", offlineTrainingWindow.defaultEnterButton));
	offlineTrainingWindow.buttons.push_back(std::make_pair("Cancel", offlineTrainingWindow.defaultEscapeButton));
...
{
	// Modal windows not supported in client 7.72 - this function is kept for compatibility
	// but won't be called since we're not sending modal windows
	Player* player = getPlayerByID(playerId);
	if (!player) {
		return;
	}
	// Function kept for protocol compatibility but not used
}
...
void Game::sendOfflineTrainingDialog(Player* player)
{
	...
	// Client 7.72 doesn't support modal windows (packet 0xFA)
	// Using text messages with numbered selection instead
	player->sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "=== Offline Training Skill Selection ===");
	player->sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Choose a skill for offline training:");
	player->sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "1. Sword Fighting and Shielding");
	player->sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "2. Axe Fighting and Shielding");
	player->sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "3. Club Fighting and Shielding");
	player->sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "4. Distance Fighting and Shielding");
	player->sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "5. Magic Level and Shielding");
	player->sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "");
	player->sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Type: /offlinetrain <number> to select (e.g., /offlinetrain 1)");
	player->sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Then click the bed again to start training.");
}
