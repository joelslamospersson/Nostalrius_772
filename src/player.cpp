...
void Player::onRemoveCreature(Creature* creature, bool isLogout)
{
	...
	if (creature == this) {
		...
		// Always update lastLogout when player is removed (for offline training)
		lastLogout = time(nullptr);
		std::cout << "[DEBUG] " << getName() << " logout - lastLogout set to: " << lastLogout << std::endl;
		...
		for (uint32_t tries = 0; tries < 3; ++tries) {
			if (IOLoginData::savePlayer(this)) {
				saved = true;
				std::cout << "[DEBUG] " << getName() << " saved successfully. lastLogout: " << lastLogout << std::endl;
				break;
			}
		}
...
uint32_t Player::checkPlayerKilling()
{
	...
	if (static_cast<int32_t>(lastDay) >= g_config.getNumber(ConfigManager::KILLS_DAY_BANISHMENT) ||
		static_cast<int32_t>(lastWeek) >= g_config.getNumber(ConfigManager::KILLS_WEEK_BANISHMENT) ||
		static_cast<int32_t>(lastMonth) >= g_config.getNumber(ConfigManager::KILLS_MONTH_BANISHMENT)) {
		return 2; // banishment!
	}

	if (static_cast<int32_t>(lastDay) >= g_config.getNumber(ConfigManager::KILLS_DAY_RED_SKULL) || 
		static_cast<int32_t>(lastWeek) >= g_config.getNumber(ConfigManager::KILLS_WEEK_RED_SKULL) || 
		static_cast<int32_t>(lastMonth) >= g_config.getNumber(ConfigManager::KILLS_MONTH_RED_SKULL)) {
		return 1; // red skull!
	}
...
{
	return std::find(modalWindows.begin(), modalWindows.end(), modalWindowId) != modalWindows.end();
}
