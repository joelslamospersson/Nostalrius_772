...
// Flag for graceful shutdown on signal
...
#ifndef _WIN32
void checkShutdownSignalTask()
{
	if (g_shutdownSignal) {
		std::cout << std::endl << "Received shutdown signal. Saving server..." << std::endl;
		...
		g_scheduler.addEvent(createSchedulerTask(1000, checkShutdownSignalTask));
	}
}
#endif
...
int main(int argc, char* argv[])
{
	...
	// Handle SIGINT (Ctrl+C) and SIGTERM (kill command) to save before shutdown
	struct sigaction shutdownHandler;
	shutdownHandler.sa_handler = [](int) {
		g_shutdownSignal = 1;
	};
	shutdownHandler.sa_flags = 0;
	sigemptyset(&shutdownHandler.sa_mask);
	sigaction(SIGINT, &shutdownHandler, nullptr);
	sigaction(SIGTERM, &shutdownHandler, nullptr);
#endif
...
#else
		// Add periodic check for shutdown signal (every second)
		g_scheduler.addEvent(createSchedulerTask(1000, checkShutdownSignalTask));
#endif
		serviceManager.run();
	} else {
