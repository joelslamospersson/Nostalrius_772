...
void Commands::saveGame(Player& player, const std::string& param)
{
	player.sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Saving server...");
	g_game.saveGameState();
	player.sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Server saved successfully.");
}

void Commands::shutdownServer(Player& player, const std::string& param)
{
	if (!param.empty()) {
		try {
			uint32_t seconds = std::stoi(param);
			player.sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Shutting down server in " + param + " seconds...");
			// Broadcast to all players
			for (const auto& it : g_game.getPlayers()) {
				it.second->sendTextMessage(MESSAGE_STATUS_WARNING, "Server will shutdown in " + param + " seconds!");
			}
			// Schedule shutdown
			uint32_t delay = seconds * 1000; // Convert seconds to milliseconds
			g_scheduler.addEvent(createSchedulerTask(delay, []() {
				g_game.setGameState(GAME_STATE_SHUTDOWN);
			}));
		} catch (...) {
			player.sendTextMessage(MESSAGE_STATUS_CONSOLE_RED, "Invalid parameter. Usage: /shutdown [seconds]");
		}
	} else {
		player.sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Shutting down server...");
		// Broadcast to all players
		for (const auto& it : g_game.getPlayers()) {
			it.second->sendTextMessage(MESSAGE_STATUS_WARNING, "Server is shutting down!");
		}
		g_game.setGameState(GAME_STATE_SHUTDOWN);
	}
}

void Commands::reloadAll(Player& player, const std::string& param)
{
	player.sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Reloading all systems...");
	...
	lua_gc(g_luaEnvironment.getLuaState(), LUA_GCCOLLECT, 0);
	player.sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "All systems reloaded successfully!");
}
