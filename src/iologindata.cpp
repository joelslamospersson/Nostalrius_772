...
bool IOLoginData::savePlayer(Player* player)
{
	...
	query << "`sex` = " << static_cast<uint32_t>(playerSex) << ',';
	...
	// Debug: Check if query ends properly
	if (finalQuery.empty() || finalQuery.find("WHERE") == std::string::npos) {
		std::cout << "[Error - savePlayer] Query construction failed! Query: " << finalQuery << std::endl;
		return false;
	}
